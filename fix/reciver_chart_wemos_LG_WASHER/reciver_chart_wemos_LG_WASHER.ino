...
void OnDataRecv(uint8_t *mac, uint8_t *incomingData, uint8_t len) {
    ...
    if (strcmp(receivedData.message, "hidupkan washer") == 0) {
        ledControl = true; // Aktifkan kontrol LED
        //mesinJalan = true;
    }
    if (strcmp(receivedData.message, "matikan mesin") == 0) {
        matikanMesin = true; // Aktifkan kontrol LED
        //mesinJalan = true;
    }
}
...
void loop() {
    ...
    if (matikanMesin && mesinJalan) {
        ...
        ledControl = false;  // Flag untuk kontrol LED
        tombolSebelumnya = HIGH;
        mesinJalan = false;
        matikanMesin = false;// Reset flag
    ...
    if (tombolSebelumnya == HIGH && tombolSekarang == LOW) {
        ...
        strcpy(myData.message, "Sensor lock");
